...
func (e *Exporter) scrape(ch chan<- prometheus.Metric) (up float64) {
	...
	if closeErr != nil {
		e.queryFailures.Inc()
		level.Error(e.logger).Log("msg", "Can't close connection", "err", closeErr)
		return 0
	}
...
func fetchHTTP(token string, sslVerify bool, tzOffset time.Duration, timeout time.Duration, logger log.Logger) func(uri url.URL) (io.ReadCloser, error) {
	...
	return func(uri url.URL) (io.ReadCloser, error) {
		dateUTC := time.Now().UTC().Add(tzOffset).Format("2006-01-02")

		level.Info(logger).Log("msg", "Scraping Wakatime", "date", dateUTC)
...
func NewExporter(uri string, token string, sslVerify bool, tzOffset time.Duration, timeout time.Duration, logger log.Logger) (*Exporter, error) {
	...
	var fetchStat func(url.URL) (io.ReadCloser, error)
	fetchStat = fetchHTTP(token, sslVerify, tzOffset, timeout, logger)
